...
public class DeleteSimulation {
	...
	@Given("^que eu tenho um id não existente$")
	...
	public void i_make_a_DELETE_request_to_the_simulation_registration_service() throws Exception {
		actions.delete_simulation_by_document(this.document);
	}

	@Then("^o codigo de resposta HTTP do serviço de deletar simulação deve ser igual (\\d+)$")
	...
	@And("^deve exibir a mensagem \"([^\"]*)\" do serviço de deletar simulação$")
	...
	@And("^deve exibir a mensagem de erro \"([^\"]*)\" do serviço de deletar simulação$")
